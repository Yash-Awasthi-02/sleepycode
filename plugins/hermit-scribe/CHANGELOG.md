...
- **Issue title format** — uses Conventional Commits (`feat(scope): ...` / `fix(scope): ...`) instead of `[hermit/{category}] ...` for proposal-backed issues. Scope is inferred from explicit target mentions in the proposal text (`plugins/<slug>/` paths or known plugin slugs) first; falls back to `_hermit_versions` in `.claude-code-hermit/config.json` only when no explicit target appears. Omitted when signals are absent or ambiguous. Ad-hoc issues pass the operator's title through unchanged.

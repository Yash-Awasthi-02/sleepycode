...
# -------------------------------------------------------
# 44. doctor-check — minimal install returns 12 checks, exits 0
# -------------------------------------------------------
workdir="$(setup_workdir)"
cd "$workdir"
...
EOF
run_test "doctor-check (minimal install, 12 checks)" bash -c \
  "node '$REPO_ROOT/scripts/doctor-check.js' '$workdir/.claude-code-hermit' >/dev/null && python3 -c \"import json; r=json.load(open('$workdir/.claude-code-hermit/state/doctor-report.json')); ids=[c['id'] for c in r['checks']]; assert ids==['config','hooks','state','cost','proposals','dependencies','permissions','docker-security','archive','reflect','scheduler','watchdog'], ids\""
cleanup

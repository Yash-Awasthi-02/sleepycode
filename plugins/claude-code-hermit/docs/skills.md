...
| `hermit-doctor` | Nine-check installation health report — config validity, hook registration, state file integrity, cost visibility, proposal health, sibling dependency ranges, file permissions, docker-security posture, sandbox capability. Use when diagnosing an install, before a release, or after suspicious behavior. | --            |

---
name: hermit-doctor
description: Returns a fourteen-check health report on the hermit installation — runtime (bun presence/version), config validity, hook registration, state file integrity, cost visibility, proposal health, sibling dependency ranges, file permissions, docker-security overlay drift, archival health, reflect loop health, scheduler/background-task health, watchdog health, sandbox capability. Use when diagnosing an install, before a release, or after suspicious behavior. Activates on messages like "/hermit-doctor", "health check", "diagnose the hermit", "what's wrong", "run diagnostic".
---

# Hermit Doctor

Runs fourteen read-only health checks against the current hermit install and surfaces the
...
2. Parse the JSON. For each of the thirteen checks in the report (`runtime`, `config`, `hooks`, `state`, `cost`,
   `proposals`, `dependencies`, `permissions`, `docker-security`, `archive`, `reflect`, `scheduler`, `watchdog`), emit one line using this format:
...
2.5. **Sandbox capability check** (fourteenth check, run after step 2):
   ...
   Add this as the fourteenth line to the summary.

3. Append a summary section to `.claude-code-hermit/sessions/SHELL.md` under a new
   `## Doctor Report (<ts>)` heading. Use the same fourteen lines from steps 2 and 2.5. Place it
   above the `## Monitoring` section so it sits with session-level context, not
   with monitoring chatter.

4. Return the fourteen lines to the caller. Cap total output at 28 lines.

## Silence policy

- If every check is `ok`, return only: `All fourteen checks passed.` Do not notify via
  channel (Tier 0). Still append to SHELL.md so the run is traceable.
- If any check is `warn` or `fail`, return the full fourteen-line summary. Channel

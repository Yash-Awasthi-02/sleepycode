...
- **reflect: file analysis runs in a shared isolated-context subagent** — Resolution Check, routine health, and procedure-capture detection are dispatched to a new `skill-eval-runner` agent (shared across all future epic skills). The runner reads only files and returns structured JSON; the main session applies all writes, frontmatter patches, and proposal routing. Eliminates the 3-verbatim-report read from the main session's inherited context, cutting per-reflect input tokens on always-on hermits. The `skill-eval-runner` agent uses `disallowedTools` only (no allowlist) to preserve the deny-list as the single enforcement mechanism. A `reflect delegation contract` in `tests/contracts.test.ts` asserts the schema block is byte-identical between `reference.md` (producer) and `SKILL.md` (consumer), preventing drift.

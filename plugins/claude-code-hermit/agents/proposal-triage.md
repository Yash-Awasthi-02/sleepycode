...
1. **Repeated pattern** — is the evidence concrete and observed more than once, across sessions?
   - **Skip for `scheduled-check/*`, `operator-request`, `current-session`, and `hermit-synthesis`** sources:
     ...
     - `hermit-synthesis`: the brainstorm pass establishes the candidate; cross-session recurrence is not required.

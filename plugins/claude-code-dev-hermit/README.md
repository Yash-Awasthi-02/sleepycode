<p align="center">
  ...
  <a href="https://github.com/gtapps/claude-code-hermit"><img src="https://img.shields.io/badge/requires-claude--code--hermit%20v1.0.21%2B-blue.svg" alt="Requires claude-code-hermit v1.0.21+" /></a>
...
- [claude-code-hermit](https://github.com/gtapps/claude-code-hermit) v1.0.21+ (installed and hatched)
...
```bash
/plugin marketplace add gtapps/claude-code-hermit # Add the marketplace
/plugin install claude-code-dev-hermit@claude-code-hermit --scope project # install the plugin
...
2. **Implement** — run `/claude-code-dev-hermit:dev-branch <description>` first (no-op if already on a feature branch), then delegate to the `implementer` agent, which works in an isolated git worktree; before overriding any implementer choice, run its tests first
...
Five operator-invoked commands cover the full dev lifecycle — feature branch creation, dev server management, log monitoring, and read-only status.

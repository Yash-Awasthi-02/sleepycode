...
- **`/dev-pr` rewrite + `pr-body-builder.js` deletion** — rewrote `skills/dev-pr/SKILL.md` (241 → ~140 lines) and deleted `scripts/lib/pr-body-builder.js` (240 lines) plus its test (326 lines). Body assembly is now inline LLM prose with explicit per-section templates (Summary / Context / Verification / Screenshots / Notes) instead of a 240-line template-merge engine. Dropped: Gate 0 quality-check (no `/dev-quality` skill anymore), Gate 0 alert-check (no watchdogs), the `--force` flag entirely (clean-tree and protected-branch are NEVER skipped, and quality/alert checks no longer exist), all `$HERMIT_AGENT_WORKTREE` always-on branching, all calls to the deleted `scripts/check-protected-branch.js` (replaced with inline bash glob via `case ... in <pattern>`), Risk section (came from quality report). Two operator-facing config knobs were silently dropped: `pr_title_format` (custom title format string — title now follows fixed priority: ticket+commit > commit > branch slug) and `pr_body_sections` (custom section ordering — order is now fixed). Operator-facing config that survives: `pr_template_path`, `pr_base_branch`, `commands.pr_create`. Project PR templates are now appended after the assembled body verbatim — no more heading-merge engine.

- **`scripts/git-push-guard.js` simplification** — rewrote the 286-line shell-aware tokenizer + per-flag option parser as ~83 lines of regex chains. Inlined `loadProtectedBranches()` and the glob-to-regex conversion (formerly in `scripts/lib/protected-branches.js`, now deleted). All exotic shell cases the old tokenizer handled (env-var prefix, `git -C path push`, `git -c key=val push`) still work — they were over-spec'd; the regex approach catches them by virtue of looking at the whole subcommand. Accepts two false positives in exchange: (a) commit messages mentioning `--no-verify` are blocked (use a different word in the message — annoying but rare), (b) deeply nested branch names like `feature/release/x` would be blocked when `release/*` is in `protected_branches` (rename the branch — also rare). Both trade-offs are documented in the test file. New policy: `--force-with-lease` is now blocked for ALL targets, not just protected (was previously allowed for feature branches) — matches the CLAUDE-APPEND.md "Never force-push" rule.
